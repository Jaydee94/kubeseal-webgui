...
def run_kubeseal(cleartext_secrets, secret_namespace, secret_name):
    if secret_namespace == None or secret_namespace == "":
...
def run_kubeseal_command(cleartext_secret_tuple, secret_namespace, secret_name):
    ...
    run_kubeseal_command = f"echo -n '{cleartext_secret}' | /kubeseal-webgui/kubeseal --raw --from-file=/dev/stdin --namespace {secret_namespace} --name {secret_name} --cert /kubeseal-webgui/cert/kubeseal-cert.pem"
    kubeseal_subprocess = subprocess.Popen([run_kubeseal_command], stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
